...
def test_detects_invisible_characters():
    ...
    res = run_detect(msg)
    assert_severity_at_least(res, "HIGH")
    # reason key may vary (token_smuggling/invisible_characters); ensure at least one trips
    assert any(r in res.get("reasons", []) for r in ["invisible_characters", "token_smuggling"])
